...
namespace SystemOverride
{
	public partial class 飞船 : RigidBody2D
	{
		...
		/// 当生命值耗尽时，飞船爆炸。
		/// </summary>
		[Export] int _最大生命值 = 100;

		/// <summary>
		/// 飞船消失前的时间，默认为7秒。
		/// </summary>
		[Export] float _消失时间 = 7.0f;

		/// <summary>
		/// 碰撞音效播放器。
		/// </summary>
		[Export] AudioStreamPlayer2D _碰撞音效;

		/// <summary>
		/// 飞船爆炸视觉效果。
		/// </summary>
		[Export] PackedScene _飞船爆炸特效;

		/// <summary>
		/// 飞船爆炸音效。
		/// </summary>
		[Export] PackedScene _飞船爆炸音效;
		...
		public int 生命值 { get; private set; }
		...
		{
			// 检查导出变量
			调试.Assert(_碰撞音效 != null, "_碰撞音效 != null");
		...
		public void TakeDamage(int damage)
		{
			if (IsDestroyed)
			{
				_碰撞音效.PitchScale = Mathf.Lerp(0.9f, 1.2f, _rng.Randf());
				_碰撞音效.Play();
				return;
			}
		...
		{
			var spaceshipExplosionSfx = _飞船爆炸音效.Instantiate<自动释放音效播放器>();
			spaceshipExplosionSfx.GlobalPosition = GlobalPosition;
			GetTree().Root.AddChild(spaceshipExplosionSfx);

			/* TODO: 队列化释放粒子效果 * /
			// 创建并实例化飞船爆炸粒子效果*/
			GpuParticles2D fx = _飞船爆炸特效.Instantiate<GpuParticles2D>();
			AddChild(fx);
